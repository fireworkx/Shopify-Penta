			...
			buildSelect(elem, select, brand, dealership, uid) {
				...
				window.penta
					...
					.then(data => {
						...
						dealerships = data.dealerships.filter(dealership => dealership.brand.toLowerCase() == brand.toLowerCase());
					...
					.then(() => {
						...
						select.addEventListener("change", () => {
							dealership = dealerships.find(dealership => dealership.name.toLowerCase() == select.value.toLowerCase());
							penta.modal.form.uid = dealership.id;
						});
					});
			},
		...
		this.sendForm = function ({ uid }) {
			...
			switch (form.id) {
				case "test-drive-form":
					...
					break;
				case "enquiry-form":
					formData.set("Message", [`Message: ${formData.get("Message")}\n`, `${Make} ${Model}`].join(""));
